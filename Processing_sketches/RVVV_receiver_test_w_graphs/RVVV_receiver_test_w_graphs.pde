...
float calculateMax(OscMessage msg, int max) {
    int []calculation = new int[max];
    for(int i=0; i<max; i++) {
      calculation[i] = msg.get(i).intValue();
    }
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if (theOscMessage.checkAddrPattern("/davide/waveform")==true) {
    ...
    if( MESSAGE_LENGTH == PRESHARED_MESSAGE_SIZE ) {
      for(int i=0; i<MESSAGE_LENGTH; i++) {
        ...
        sw.changeAmp(map(calculateMax(theOscMessage, MESSAGE_LENGTH), 0, 30, 0, 10), 0);
      }
    }
  ...
  if(theOscMessage.checkAddrPattern("/change_scene")==true) {
    changeScene();
    return;
  }
